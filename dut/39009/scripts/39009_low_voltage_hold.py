...
# 39009 command framing:
# - Command 0x00 clears command multiplex / keepalive.
# - Command 0x52 with byte2=0x54 enables DPLF1/DPLF2/DPLTx.
CTRL_CMD0_PAYLOAD = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
CTRL_ENABLE_PAYLOAD = [0x52, 0x54, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "39009 Low Voltage Hold behavior verifier with direct Python control of "
    ...
    parser.add_argument("--expected-sa", type=_parse_int_auto, default=_parse_int_auto("0xE0"), help="Expected DUT SA.")
    parser.add_argument("--controller-sa", type=_parse_int_auto, default=_parse_int_auto("0xD1"), help="Tester SA.")
    parser.add_argument(
        "--fault-code",
        type=int,
        default=2,
        help="Expected low-voltage fault code (2).",
    )

    parser.add_argument("--pat-dbc", default="", help="Optional PAT.dbc path override.")
    parser.add_argument("--uut-dbc", default="", help="Optional 39009 DBC path override.")
    ...
    parser.add_argument("--low-fault-voltage", type=float, default=10.5, help="Voltage used for low-voltage-fault phase.")
    parser.add_argument(
    ...
    parser.add_argument("--post-enable-quiet-sec", type=float, default=0.5, help="No CTRL TX time after one-shot Command=0x52.")
    parser.add_argument("--enable-timeout-sec", type=float, default=8.0, help="Timeout waiting for STAT traffic after enable.")
    ...
    parser.add_argument(
        "--hold-stop-min-silence-sec",
        type=float,
        default=1.0,
        help="Minimum observed silence during hold to consider CAN stopped.",
    )
    ...
    if args.hold_stop_min_silence_sec <= 0:
        raise ValueError("--hold-stop-min-silence-sec must be > 0.")
...
def drain_bus(bus, drain_sec=0.25):
    """Discard queued CAN frames so the next observe window uses fresh traffic."""
    end = time.monotonic() + float(max(0.0, drain_sec))
    dropped = 0
...
def _decode_vbat_from_dplf2(data):
    if len(data) < 4:
        return None
    # 39009 DPLF2.VBAT is a 16-bit field (byte2..3). Empirically decivolts.
    raw = int(data[2]) | (int(data[3]) << 8)
    return raw * 0.1
...
def ensure_stat_seen(bus, module_sa, controller_sa, timeout_s, cmd0_period_s):
    deadline = time.monotonic() + float(timeout_s)
    ...
    while time.monotonic() < deadline:
        ...
        stat_frames += 1
        if stat_frames > 0:
            return True, stat_frames
    return False, stat_frames
...
def observe_window(
    ...
    stat = uut_db.get_message_by_name("STAT")
    ...
    while time.monotonic() < deadline:
        ...
        if pgn == PGN_PROPRIETARY_A:
            out["stat_frames"] += 1
            try:
                decoded = stat.decode(data, decode_choices=False)
            except Exception:
                decoded = None
            if decoded is not None:
                code = int(decoded.get("Active_Fault_Code", 0))
...
def hold_can_stopped_check(observation, hold_stop_min_silence_sec):
    worst_gap = max(float(observation["max_inter_gap"]), float(observation["trailing_gap"]))
    if worst_gap >= float(hold_stop_min_silence_sec):
        return True, f"observed silence {worst_gap:.3f}s"
    return False, f"observed silence {worst_gap:.3f}s is < {float(hold_stop_min_silence_sec):.3f}s"
...
def main():
    ...
    uut_dbc_path = resolve_path(args.uut_dbc, "dbc", "39009-561.dbc")
    ...
    if pat_dbc_path is None or uut_dbc_path is None or rigol_helper is None:
        print("[FAIL] Missing required file(s): PAT.dbc, 39009-561.dbc, or rigol helper.")
        return emit_result(False)

    print("39009 Low Voltage Hold behavior verifier")
    ...
    print(f"CTRL behavior: one-shot Command=0x52, quiet {args.post_enable_quiet_sec:.1f}s, then Command=0 polling")
    print(f"Fault acceptance in low-fault phase: code {args.fault_code}")
    ...
    behavior_checks = {
        ...
        "CAN transmission stops during low-voltage hold": None,
    ...
    try:
        ...
        print("Setting up PAT fixture safe routing (PAT supply remains off)...")
        pat.set_signals(
            {
                "LdRemote": 1,
                "LdCurrentSet": 0,
        ...
        print("Sent one-shot Command=0x52 enable.")
        time.sleep(max(0.0, float(args.post_enable_quiet_sec)))

        ok_stat, stat_frames = ensure_stat_seen(
            uut_bus,
            module_sa,
            args.controller_sa,
        ...
        print(f"STAT frames observed after quiet: {stat_frames}")
        if not ok_stat:
            print("[FAIL] Did not observe STAT traffic after quiet+Command=0 polling.")
            return emit_result(False)
        print("[PASS] STAT traffic observed after enable.")
        ...
        print("")
        print(
            "Phase 3: Low-voltage fault check "
            + f"(<{args.low_fault_threshold_voltage:.1f}V for >= {args.low_fault_min_sec:.1f}s, operation should continue)."
        )
        if not set_rigol_voltage(args, rigol_helper, args.low_fault_voltage, "on"):
            ...
            dropped = drain_bus(uut_bus, 0.30)
            if dropped > 0:
                print(f"[INFO] Dropped {dropped} stale CAN frame(s) before low-fault observation.")
        ...
        if not set_rigol_voltage(args, rigol_helper, args.hold_voltage, "on"):
            ...
            behavior_checks["CAN transmission stops during low-voltage hold"] = False
            overall_pass = False
        else:
            dropped = drain_bus(uut_bus, 0.30)
            if dropped > 0:
                print(f"[INFO] Dropped {dropped} stale CAN frame(s) before hold observation.")
            ...
            hold_stop_ok, hold_stop_note = hold_can_stopped_check(
                hold_behavior_obs,
                args.hold_stop_min_silence_sec,
            )
            behavior_checks["Below 8.0V for 8s enters low-voltage hold"] = hold_stop_ok
            behavior_checks["CAN transmission stops during low-voltage hold"] = hold_stop_ok
            print(f"[INFO] Hold-phase fault codes observed: {sorted(hold_behavior_obs['fault_codes'])}")
            if hold_stop_ok:
                print(f"[PASS] CAN transmission stopped in hold ({hold_stop_note}).")
            else:
                print(f"[FAIL] CAN did not show hold-stop behavior ({hold_stop_note}).")
                overall_pass = False
            if not behavior_checks["Below 8.0V for 8s enters low-voltage hold"]:
                overall_pass = False
        ...
        if not set_rigol_voltage(args, rigol_helper, args.nominal_voltage, "on"):
            ...
            dropped = drain_bus(uut_bus, 0.30)
            if dropped > 0:
                print(f"[INFO] Dropped {dropped} stale CAN frame(s) before recovery observation.")
            ...
            behavior_checks["Voltage recovery exits low-voltage hold"] = continuity_ok
            if behavior_checks["Voltage recovery exits low-voltage hold"]:
                print(f"[PASS] Recovery phase resumed operation (last fault code={last_fault}; {continuity_note}).")
            else:
                print(
                    "[FAIL] Recovery phase: "
                    + f"continuity_ok={continuity_ok} ({continuity_note}); "
